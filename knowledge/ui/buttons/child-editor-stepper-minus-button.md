---
type: UI Button
title: Child Editor Stepper Minus Button
description: Template minus button for years, months, and weight steppers.
...
`Stepper` clamps and calls `onChange(Math.max(min, value - step))`.

# Instances And Placement

The same minus button template appears in each stepper: years, months, and
